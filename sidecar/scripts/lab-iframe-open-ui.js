/**
 * Drive lab UI as the DOM apply surface for blueprint iframe-open.
 * CLI lab:run has no DOM client — nested iso is unproven without this.
 * Run: node scripts/lab-iframe-open-ui.js
 */
const { spawn } = require('node:child_process');
const path = require('node:path');
const { chromium } = require('patchright');

const PORT = process.env.SPECULUM_LAB_PORT || '4098';
...
async function runIframeOpenUi() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => [...document.querySelectorAll('#blueprint option')].some((o) => o.value === 'iframe-open'),
      null,
      { timeout: 15_000 },
    );
    await page.selectOption('#blueprint', 'iframe-open');
    ...
    while (Date.now() < deadline) {
      ...
      if (/finished with .* fail/i.test(hint)) {
        ...
        throw new Error(
          `iframe-open failed hint=${hint} chip=${chip} dossier=${dossier}\n${verdicts}\n${activity}`,
        );
      }
      if (/no fails in summary/i.test(hint)) {
        console.log(`[iframe-open-ui] pass dossier=${dossier.trim()}`);
        return;
      }
    ...
    throw new Error('iframe-open UI run timed out');
  } finally {
    await browser.close();
  }
}
...
async function main() {
  ...
  try {
    ...
    await runIframeOpenUi();
  } catch (err) {
    console.error('[iframe-open-ui] failed', err);
