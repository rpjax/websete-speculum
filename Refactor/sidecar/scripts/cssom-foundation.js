/**
 * CSSOM foundation lab gate: scenario runner, then low-volume cssom-scale --iso.
 * npm extra-args must not ride on build (Windows).
 */
...
let failed = 0;
failed += runNode([
  path.join(root, 'dist', 'browser', 'mirror', 'projection', 'lab', 'cssomFoundationRun.js'),
])
  ? 1
  : 0;

const runCli = path.join(root, 'dist', 'browser', 'mirror', 'projection', 'lab', 'runCli.js');
const scale = [
...
for (const [url, duration] of scale) {
  const st = runNode([runCli, url, duration, 'iso']);
  if (st) failed += 1;
}
