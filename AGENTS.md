...
4. [docs/diagnostics.md](docs/diagnostics.md) — Act→Assert contracts (if the change touches observability or session tests).
5. [Speculum.Api.SessionsTest.Tests/MATRIX.md](Speculum.Api.SessionsTest.Tests/MATRIX.md) — coverage truth for SessionsTest CI.
...
- **No ad-hoc / workaround code — ever. Carved in stone.** If the designed algorithm fails (stream, table apply, resync), **fix that algorithm**. Never reintroduce a banned path (e.g. full DomMap dump / “bootstrap” after stream seed on cold) “just to get green / hopdiag / a live surface”. Ad-hoc is **strictly forbidden**; green via workaround is a product defect. Same law: [docs/engineering-standards.md](docs/engineering-standards.md), [docs/page-projection/spec/acceptance.md](docs/page-projection/spec/acceptance.md) (T3 / V4), [CONTRIBUTING.md](CONTRIBUTING.md). **PageProjection spec index:** [docs/page-projection/spec/README.md](docs/page-projection/spec/README.md) — do not implement from `docs/page-projection/archive/`. Session contracts: [docs/page-projection/spec/browser-session.md](docs/page-projection/spec/browser-session.md). Lab/telemetry/probes: [docs/page-projection/spec/observability.md](docs/page-projection/spec/observability.md).
- **PageProjection accept = 1:1** with the original site (Projected ≈ opening Virtual’s target in a normal browser). **DOM** = numerical/state 1:1; **CSSOM live** = perceived/eventual (not 60 Hz lockstep). Protocol recovery / green smoke alone never prove accept — see [docs/page-projection/spec/acceptance.md](docs/page-projection/spec/acceptance.md).
- **Done = last hop live**, not files on disk. Encoder unused, hook-only-as-checkout-script, IPDL with no native emit, or golden/FakeBrowser green while the product path is dead = **incomplete**. Do not mark the todo complete. Say incomplete in the first sentence. Cursor: [`.cursor/rules/speculum-done-bar.mdc`](.cursor/rules/speculum-done-bar.mdc).
...
## SessionsTest harness (when touching CI tests)

- Stack: `deploy/compose/docker-compose.sessions-test.yml` + seed script + `Speculum.Api.SessionsTest.Tests`.
- Prefer catalogued events + probes; never prove session truth with `200` / `ok: true` alone.
- Extend [MATRIX.md](Speculum.Api.SessionsTest.Tests/MATRIX.md) when coverage depth changes. Never weaken asserts to get green ([docs/assert-failure-policy.md](docs/assert-failure-policy.md)).
...
Product code lives at repo root (`Speculum.Api/`, `web/`, `sidecar/`, `packages/`, `deploy/`, …). The pre-promotion SPA is kept only under `Legacy/web/`.
