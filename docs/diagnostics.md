...
The `Telemetry` domain publishes one periodic composite event, **`Telemetry.SampleCollected`** (capability `Metric`, persisted), on a single global interval (`telemetry.intervalSeconds`). Overlaying host × sessions × sidecar × profiles × Journal × Docker on one time axis is how the sample *tells a story* (symptom → signal). Each section is emitted only when its toggle is on; a missing section means "not collected".
...
| `sessions` | `telemetry.sessions.enabled` | `total`, `live`, `avgFps`/`minFps`/`maxFps`, `capacityMax`, `capacityUsedPct`; `liveSessionIds[]?` (`includeSessionIds`), `sessions[]?` (`includePerSession`; each item carries `sessionId`, `profileId`, `jsBridgeEnabled`, `connectionOpen`, `uptimeMs`, `fps?`, `urlHost?`) |
| `sidecar` | `telemetry.sidecar.enabled` | `process?`, `eventLoop?`, `chrome?`, `queues?`, `sessions?`. Queue section exposes `videoDepth`, `audioDepth`, `consoleDepth`, optional `inputDepth`, and optional cumulative `droppedTotal`. Session summary exposes `registered`, `open`, `faulted`, `faultedSessionIds[]?` |
| `profiles` | `telemetry.profiles.enabled` | `total`, optional `storageBytes` |
| `journal` | `telemetry.journal.enabled` | `queueDepth`, `droppedTotal`, `degraded`; optional `persistFailures`, `guaranteedAdmissionFailures`, `queuePressureActive`, `persistDegraded`, `drainRunning`, `admissionOpen` (`includePressure`) |
| `docker` | `telemetry.docker.enabled` | `runtime?`, `containers?`; runtime includes engine/version counts, containers include state plus optional CPU/memory/network stats for running containers |

Symptom → signal coverage: API memory leak (`apiProcess.memoryUsed`/`gcGen2`/`gcHeap` rise vs flat `sessions.live`); VPS saturation (`host.cpuUsage`/`host.memoryUsed÷memoryTotal` high); render regression (`sessions.avgFps`/`minFps` fall vs flat `host.cpuUsage`, with `sidecar.queues.*` helping separate relay pressure from browser pressure); thread starvation (`apiProcess.threadPoolQueued` up, `threadPoolBusy` at ceiling); session-capacity saturation (`sessions.capacityUsedPct` → 100); sidecar instability (`sidecar.sessions.faulted` up, correlates with `Motor.SidecarFaulted`); Journal stress (`journal.persistFailures`, `journal.queueDepth`, `journal.droppedTotal`). Aggregate-only by default; identity (`liveSessionIds`/`sessions`/`faultedSessionIds`/`urlHost`) is opt-in and still governed by read-time redaction.

When `telemetry.sessions.includePerSession` is on, each live session's slice is *also* mirrored as its own **`Telemetry.SessionSampleCollected`** event scoped by `connectionId` (payload = one `sessions[]` entry). Same data as the composite's `sessions.sessions[]`, no extra registry pass — it exists so a session's samples plot inside that session's story lane, not only in the global composite. Covered by `TelemetryEmitterTests`.
