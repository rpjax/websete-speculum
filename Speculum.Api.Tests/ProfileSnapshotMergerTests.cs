...
public sealed class ProfileSnapshotMergerTests
{
    ...
    private sealed class InMemorySnapshotStore : IBrowserSnapshotStore
    {
        ...
        public Task<BrowserSnapshotRecord?> TryLoadAsync(string sessionId, CancellationToken ct = default)
        {
            _rows.TryGetValue(sessionId, out var row);
            return Task.FromResult(row);
        }

        public Task SaveAsync(string sessionId, byte[] profileBlob, string lastUrl, CancellationToken ct = default)
        {
            SaveCount++;
            LastSavedBlob = profileBlob;
            LastSavedUrl  = lastUrl;
            _rows[sessionId] = new BrowserSnapshotRecord
            {
                SessionId   = sessionId,
        ...
        public Task SeedAsync(string sessionId, byte[] blob, string url, DateTimeOffset updatedAt)
        {
            _rows[sessionId] = new BrowserSnapshotRecord
            {
                SessionId   = sessionId,
        ...
        public Task<bool> DeleteAsync(string sessionId, CancellationToken ct = default) => Task.FromResult(false);
