...
class BrowserPool {
    ...
    tryRefill() {
        if (this.disposed)
            return false;
        if (this.ready.length + this.launching >= this.size)
            return false;
        const minIntervalMs = 1000 / this.refillPerSec;
        const nowMs = this.now();
        if (nowMs - this.lastRefillAt < minIntervalMs)
            return false;
    ...
    async acquire(contextOptions) {
        ...
        if (pooled)
            this.tryRefill(); // opportunistic; startAutoRefill() covers steady-state production
