"use strict";
(() => {
  // browser/mirror/projection/inpage/binaryFrameEncoder.ts
  ...
  // browser/mirror/projection/inpage/domMutationAccumulator.ts
  var DomMutationAccumulator = class {
    ...
    /**
     * O(1) pointer swap. Returns the Frozen buffer to flush.
     * If a prior Frozen was deferred, do not swap again until accepted (E-02).
     */
    swap() {
    ...
    /** Merge Frozen back into Active — build not ready after swap. */
    reclaimFrozen() {
  ...
  // browser/mirror/projection/inpage/domNodeTable.ts
  var DomNodeTable = class {
    ...
    /** Reverse map size — soak-tested by PP-ID-4 to stay bounded. */
    get size() {
      return this.byKey.size;
    }
    /**
     * Insert (or return existing key for) a DOM node.
     * Keys are monotonic from 1.
     */
    allocate(node) {
    ...
    /** Peek key — never inserts. Unknown → `NONE_DOM_NODE_KEY`. */
    keyOf(node) {
    ...
    /** Look up the DOM node for a key. Unknown / GC'd / `0` → `undefined`. */
    get(key) {
    ...
    /** Remove one node from both directions without bumping generation. */
    release(node) {
    ...
    /**
     * Real top-level Document swap: clear the table, bump generation.
     * WeakMap has no clear() — replace the forward map.
     * Keys keep counting up (never reused across generations either).
     */
    bumpGeneration() {
  ...
  // browser/mirror/projection/inpage/domMutationObserver.ts
  var OBSERVE_OPTIONS = {
    subtree: true,
    childList: true,
    attributes: true,
  ...
  var DomMutationObserver = class {
    ...
    /** Test / pierce helper — same path as the live callback. */
  ...
  // browser/mirror/projection/inpage/frameBuilder.ts
  ...
  // browser/mirror/projection/inpage/frameEmitter.ts
  ...
  // browser/mirror/projection/inpage/frameClock.ts
  ...
  // browser/mirror/projection/inpage/loopbackFrameTransport.ts
  ...
  var LoopbackFrameTransport = class {
    ...
    /** Current destination URL, if configured. */
    get destinationUrl() {
      return this.url;
    }
    get isOpen() {
    ...
    /**
     * Connect (or reconnect) to the session data-plane URL.
     * Safe to call again — previous socket is closed first.
     */
    open(url) {
      this.close();
      this.url = url;
  ...
  // browser/mirror/projection/inpage/main.ts
  (() => {
