/**
 * Turnstile sheet dump calibration — runs eneba-turnstile UI flow and captures CSSOM sheet dump.
 * Run: npm run lab:turnstile-sheet-dump
 */
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      if (finished) {
        ...
        console.log(`[turnstile-sheet-dump] done hint=${hint.trim()}`);
...
function logSheetArtifacts(dossierDir) {
  const virtual = path.join(dossierDir, 'probes', 'turnstile-sheet-dump-virtual.json');
  const projected = path.join(dossierDir, 'probes', 'turnstile-sheet-dump-projected.json');
  const combined = path.join(dossierDir, 'probes', 'cssom-sheet-dump.json');
  for (const p of [virtual, projected, combined]) {
    if (fs.existsSync(p)) {
      console.log(`[turnstile-sheet-dump] artifact ${p}`);
    }
  }
}
...
async function main() {
  ...
  try {
    ...
    logSheetArtifacts(dossier);
    console.log(`[turnstile-sheet-dump] dossier ${dossier}`);
  } catch (err) {
    console.error('[turnstile-sheet-dump] failed', err);
