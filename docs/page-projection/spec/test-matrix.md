...
| `PP-F-3` | Open shadow: Projected host has a real `ShadowRoot`; light `childNodes` and `shadowRoot.childNodes` match Virtual (browser slots). **Not** flattened on the wire. [shadow.md](shadow.md). Until shipped: fail explicit unsupported. | WP3 |
| `PP-F-4` | **Split.** Closed shadow: **NIT**, fail unsupported now ([shadow.md](shadow.md)). Cross-origin iframe: **OPEN-6**, fail unsupported. Do not fake a pass. Not pierce. | WP3 |
