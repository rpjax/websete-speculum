...
export function diffDiagnosticsConfig(current: DiagnosticsOptions, pending: DiagnosticsOptions): ConfigChange[] {
  ...
  diffToggle(changes, 'Telemetry · Host', current.telemetry.host.enabled, pending.telemetry.host.enabled)
  ...
  diffNumber(changes, 'Host sample interval', current.probe.hostSampleIntervalMs, pending.probe.hostSampleIntervalMs, (v) => `${v}ms`)
