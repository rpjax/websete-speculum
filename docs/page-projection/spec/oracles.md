...
| **O2** | **Structural self-check** | Compare Virtual live DOM against the client tree; MUST be isomorphic after settle. Full comparison is CI/debug/lab only (O(n) — would violate E1 in production). Production cheap variant: node count + tableHash vs client-reported hash. | Coherent probe: `flushProjectionSnapshot` + client `requestSnapshot`. Local table×DOM inside that turn. Table×table via `ReplicatedTableDigest`. Tree×tree via `structuralDiff.ts`. `FrameInvariantMonitor` stays **wire-only** (no telemetry asserts). CLI without a client apply surface: Virtual O2 runs; client legs `skipped`. |
