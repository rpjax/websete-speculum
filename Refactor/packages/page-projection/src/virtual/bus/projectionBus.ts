/**
 * postMessage bus — control (RPC) + loose events (frames, resync request, telemetry).
...
export type ProjectionBusOptions = {
  ...
  /** Root runtime carries resync to the sidecar control channel. */
  sendSidecarResync?: (req: ResyncRequestEvent) => void;
};
...
export class ProjectionBus {
  ...
  private readonly sendSidecarResync: ((req: ResyncRequestEvent) => void) | null;
  private corr = 1;
  ...
  emitResyncRequest(req: ResyncRequestEvent): void {
  ...
  private async handleControl(env: ControlEnvelope, event: MessageEvent): Promise<void> {
    if (env.type === 'response') {
      ...
      if (pending.kind === 'number') {
        pending.resolve(env.ok === true ? env.value : undefined);
        return;
      }
