...
export function ProfileDeletePage() {
  ...
  const remove = async () => {
    ...
    try {
      ...
      navigate('/w7s/admin/profiles', { replace: true, state: { message: 'Profile deleted' } })
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : 'Unable to delete profile.'
      if (/live session/i.test(message)) { navigate(`/w7s/admin/profiles/${encodeURIComponent(profileId)}`, { replace: true, state: { message } }); return }
      setError(message)
  ...
  return <div className="mx-auto max-w-2xl space-y-6">
    ...
    <ConfirmDestructive open onOpenChange={(open) => { if (!open) navigate(`/w7s/admin/profiles/${encodeURIComponent(profileId)}`) }} title="Delete profile" body="This removes persisted browser state for this identity. This cannot be undone." confirmLabel="Delete permanently" onConfirm={remove} submitting={submitting} />
  </div>
}
