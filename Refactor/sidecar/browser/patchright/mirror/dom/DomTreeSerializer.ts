...
export type DomSelectorJson = {
  kind: 'element' | 'childAt';
  query: string;
  index?: number;
};

/** Exclusive operation body — shape is chosen by `plane` + `operation`. */
export type PageProjectionEmitPayload = Record<string, unknown>;

export type PageProjectionDiffEmit = {
  sequence: number;
  generation: number;
  plane: 'dom' | 'cssom';
  /**
  ...
  operation: string;
  timestampMs: number;
  /** UTF-8 JSON of the operation payload only. */
  body: Uint8Array;
};
...
export function encodeDomBody(payload: unknown): Uint8Array {
  return Buffer.from(JSON.stringify(payload ?? {}), 'utf8');
}

export function decodeDomBody(bytes: Uint8Array): unknown {
 ...
 */
export const PAGE_PROJECTION_PAGE_SCRIPT = `
(() => {
