...
class H264Capture {
    ...
    static start(displayNum, width, height, onFrame) {
        ...
        return new Promise((resolve, reject) => {
            ...
            capture._spawn(displayNum, width, height, (firstBuf) => { onFrame(firstBuf); settle(() => resolve(capture)); }, (err) => settle(() => reject(err)), onFrame);
        });
    }
    _spawn(displayNum, width, height, onFirstFrame, onStartError, onFrame) {
        ...
        let firstFrame = true;
        framer.on('data', (frame) => {
            if (this._stopped)
                return;
            const encoded = (0, Protocol_1.encodeH264Frame)(frame.isKeyframe, frame.data);
            if (firstFrame) {
                firstFrame = false;
                onFirstFrame(encoded);
                return;
            }
            onFrame(encoded);
        });
        proc.stderr.on('data', (chunk) => {
            if (!this._stopped)
                console.error('[H264Capture]', chunk.toString().trimEnd());
        });
        proc.on('error', (err) => {
        ...
        proc.on('close', (code, signal) => {
            if (firstFrame) {
                onStartError(new Error(`[H264Capture] FFmpeg exited before producing any frames ` +
...
exports.H264Capture = H264Capture;
/**
 ...
 */
class H264AnnexBFramer extends stream_1.Transform {
    _buf = Buffer.allocUnsafe(512 * 1024);
    _len = 0;
    _frameNals = [];
    _isKeyframe = false;
    _hasSlice = false;
    static MAX_BUF = 16 * 1024 * 1024;
    constructor() {
    ...
    _transform(chunk, _enc, cb) {
        ...
        if (needed > H264AnnexBFramer.MAX_BUF) {
            this._len = 0;
            cb();
            return;
        }
        ...
        while (true) {
            ...
            if (!sc2)
                break; // NAL not yet complete — need more data
        ...
        if (consumed > 0) {
            ...
            if (rem > 0)
                this._buf.copy(this._buf, 0, consumed, this._len);
            this._len = rem;
        }
        cb();
    }
    ...
    _findSC(from) {
        ...
        for (let i = from; i < end; i++) {
            if (this._buf[i] !== 0 || this._buf[i + 1] !== 0) {
                i++;
                continue;
            }
            // Have 00 00 at i, i+1
