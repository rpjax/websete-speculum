...
| Id | Symptom | Notes |
|----|---------|-------|
| **GECKO-INPUT-NODE-TARGET** | Eneba: cookie/header/`<a>` near the top work; store cards, `Ver ofertas`, sidebar rows do not. Client sends `down/up` (`nodeId` set). Virtual often does not navigate. | **Cause:** `SpeculumInput` click used `PresShell::HandleEvent` on the **root frame** (coordinate hit-test). Protocol is `nodeId` + local% ([input.md](input.md)). **Fix:** click → `HandleEventWithTarget` on the named element’s primary frame (local% in that box). Key → focused element in this document + `keypress` on character/Enter. **Lab 2026-09-17 session `446ff61c8fab` after mach:** store card Zomboid navigated; `Ver ofertas` opened 16 ofertas; `Comprar agora` → checkout cart. DESYNC 0 through that path. **Keyboard (2026-09-18):** residual was typing with `value` stuck `""` — `FillKeyEvent` lacked `mKeyCode`/`mCharCode`, and WidgetKeyboardEvent alone does not mutate `<input>`/`<textarea>` (Chromium used `keyboard.down` / `Input.insertText`). **Fix in tree:** complete keyCode/charCode + `SetUserInput` insert/Backspace/Delete on focused control (`SpeculumInput.cpp`); Live `intentToWire` preserves key `contextId`. Oracle: `gecko-engine/devpath/_diag-gecko-keyboard-oracle.mjs`. **Effect proof:** needs mach rebuild + lab/Live assert (`valueLen>0` after `keyDown` `"a"`) — wired, unproven until that hop. |
