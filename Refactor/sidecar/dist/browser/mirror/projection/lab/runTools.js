...
function createRunCollectors() {
    ...
    return {
        ...
        observeTelemetry(msg) {
            ...
            if (msg.kind === 'desynced')
                desyncs.push(msg);
        },
    };
}
async function executeLabRun(hooks, req, collectors) {
    ...
    await new Promise((resolve) => setTimeout(resolve, req.durationMs));
    ...
    if (cpuStarted && hooks.session.stopCpuProfile) {
        ...
        if (stop.ok && stop.profileBytes) {
            cpuRaw = JSON.parse(new TextDecoder().decode(stop.profileBytes));
            cpuSummary = (0, cpuProfile_1.summarizeProfile)(cpuRaw, 20);
    ...
    if (!req.structuralDiff) {
        ...
        const isoOnly = await (0, isomorphism_1.runIsomorphism)({
    ...
    const written = await (0, runReport_1.writeRunReport)(req.outDir ?? (0, runReport_1.defaultLabRunsDir)(), report, cpuRaw);
