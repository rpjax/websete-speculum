...
import { Activity, ArrowRight, Boxes, CheckCircle2, FileCode2, RefreshCw, Server, Settings2, UsersRound } from 'lucide-react'
...
const sections: ReadonlyArray<{ key: string; description: string; href?: string }> = [
  ...
  { key: 'Scripting', description: 'Injected script policy and sources.', href: '/admin/scripts?tab=injections' },
...
const shortcuts = [
  { label: 'Sessions', description: 'Inspect live sessions', href: '/admin/sessions', icon: Activity },
  { label: 'Profiles', description: 'Persisted browser identities', href: '/admin/profiles', icon: UsersRound },
  { label: 'Scripts', description: 'Library and page injections', href: '/admin/scripts', icon: FileCode2 },
  { label: 'Configurations', description: 'Engine sections', href: '/admin/configurations', icon: Settings2 },
  { label: 'Host resources', description: 'Capacity and shm', href: '/admin/host-resources', icon: Server },
  { label: 'Diagnostics', description: 'Observe and govern', href: '/admin/diagnostics', icon: Boxes },
]
  ...
  return <AdminPage width="overview" className="space-y-6">
    ...
    {status && sessions ? <>
      ...
      <section className="grid gap-3 sm:grid-cols-2" aria-label="Operator summary">
        ...
        <StatCard label="Live sessions" value={sessions.length} icon={<Activity className="h-4 w-4" />} sub={<Link to="/admin/sessions" className="underline">Inspect live sessions</Link>} />
      </section>
      {!status.operational ? <DataCard className="p-4"><div className="space-y-4">
        ...
        <MetaRow>{status.missing.map((section) => <IdChip key={section} id={section} href={`/admin/configurations/${encodeURIComponent(section)}`} alwaysShort className="bg-warning/10 text-warning" />)}</MetaRow>
        <NextBestAction title="Continue setup" body="Complete missing sections so sessions can start." ctaLabel="Open setup" href="/setup" tone="warning" />
      </div></DataCard> : null}
      <section className="grid gap-4 lg:grid-cols-2">
        <DataCard className="p-4">
          <div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">Live sessions</h2><p className="mt-1 text-sm text-muted-foreground">Currently attached browser sessions.</p></div><Button asChild size="sm" variant="ghost"><Link to="/admin/sessions">View all</Link></Button></div>
          {sessions.length ? <div className="mt-4 divide-y divide-border">{sessions.slice(0, 5).map((session) => <div key={session.sessionId} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0 space-y-1"><IdChip id={session.sessionId} href={`/admin/sessions/${encodeURIComponent(session.sessionId)}`} /><MetaRow><StatusPill label={session.connectionOpen ? 'Open' : 'Closed'} tone={session.connectionOpen ? 'success' : 'neutral'} /><span className="text-xs text-muted-foreground">{formatDuration(session.uptimeMs)}</span></MetaRow></div><Button asChild size="sm" variant="outline"><Link to={`/admin/sessions/${encodeURIComponent(session.sessionId)}`}>Open</Link></Button></div>)}</div> : <div className="mt-4"><EmptyState title="No live sessions" body="No live sessions — normal when idle." tone="reassure" /></div>}
        </DataCard>
        <DataCard className="p-4">
