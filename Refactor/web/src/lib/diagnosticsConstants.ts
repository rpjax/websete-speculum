...
export const DOMAIN_DESCRIPTIONS: Record<string, string> = {
  ...
  Telemetry: 'Composite periodic snapshot — machine, API process, motor, sidecar, persistence, pipeline',
...
> = {
  Development: {
    ...
    telemetry: {
      ...
      sessions: { enabled: true, includeSessionIds: true, includePerSession: true, includeUrlHost: true },
      sidecar: {
        enabled: true,
        includeProcess: true,
      ...
      profiles: { enabled: true, includeStorageBytes: true },
      journal: { enabled: true, includePressure: true },
      docker: { enabled: true, endpoint: 'unix:///var/run/docker.sock', includeRuntime: true, includeContainers: true, timeoutMs: 5000 },
    },
    storage: {
  ...
  Production: {
    ...
    telemetry: {
      ...
      sessions: { enabled: true, includeSessionIds: true, includePerSession: false, includeUrlHost: true },
      sidecar: {
        enabled: true,
        includeProcess: true,
        includeEventLoop: true,
