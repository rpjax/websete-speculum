...
| **OPEN-7** | **Found 2026-08-14 (external reconciliation pass, `RECONCILIATION.md` §5) — `ReplicatedTable.insertBatch` does not repair the reverse `nextSiblingOf` link on insert-before-existing** | **Confirmed still present** by direct code read the same day (`models/replicatedTable.ts`, `insertBatch`/`relinkPrevSibling`) — not yet fixed. `insertBatch`'s final step for `before !== NONE` calls `relinkPrevSibling(before, prev)`, which sets `before`'s own `prevSibling` field but never sets the derived reverse index `nextSiblingOf.set(prev, before)` (contrast `linkAfter`, which sets both directions). A later `REMOVE` of `prev` (the last id inserted by that batch) then reads `nextSiblingOf.get(prev)` as absent/`NONE` and skips repairing `before.prevSibling`, leaving it pointing at the now-detached, dropped id — a **hashed** field (§1.4 `rowHash`), so it silently violates **P0** ("the producer holds a copy, not a belief") rather than failing loud. Producer and client run the identical `ReplicatedTable` code, so both corrupt identically and `preTableHash`/`CHECK` cannot catch it by construction — this is exactly the class of bug §6's two-phase apply is powerless against (it validates *consistency*, not *correctness*, of the derived indices). Not currently DOM-visible: materialization (§6) applies from each op's own `before` operand, not from the table's derived links, so the projected surface stays correct visually; a subsequent resync (§5.8) heals it via `emitResyncFrame`'s from-scratch pass 2 (append-only inserts read fresh from `node.childNodes`). Falsifier (from `RECONCILIATION.md`): `INSERT(P, before=X, [A,L]); REMOVE(P,[L])` should leave `getRow(X).prevSibling === id(A)`; today it stays `id(L)`. **Not sealed as a design question** — this is a one-line implementation defect in an otherwise-correct design (`linkAfter`'s own pattern is the fix: also set `nextSiblingOf[last] = before` when `before !== NONE`), tracked here rather than silently fixed because it is a live P0 violation in code already exercised by the lab's smoke/unit gates and must not be waved through as "resync papers over it" — recovery healing a bug is not the same as the bug not existing (`.cursor/rules/speculum-page-projection-acceptance.mdc`: no ad-hoc workaround in place of fixing the algorithm). Blocks nothing already shipped (Stages 1-4 gates do not happen to exercise this shape — `prepend-stress.html` evicts tail-first, per `RECONCILIATION.md`), but **must close before production cutover** (`work-order.md`, "Path to M1 cutover"). |
