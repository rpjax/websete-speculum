...
interface SmokeConsoleValueProps {
  value: unknown
  name?: string
  depth?: number
}
 ...
 */
export function SmokeConsoleValue({ value, name, depth = 0 }: SmokeConsoleValueProps) {
          ...
          {entries.length === 0 ? (
            ...
            entries.map(({ key, child }) => (
              <div key={key} className="py-px">
                <SmokeConsoleValue value={child} name={key} depth={depth + 1} />
              </div>
            ))
          )}
        </div>
