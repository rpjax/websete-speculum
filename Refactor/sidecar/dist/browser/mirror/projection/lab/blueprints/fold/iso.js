...
function foldCssomPaintBoundary(cascade, opts) {
    const verdicts = [];
...
function foldIsoJournal(iso, opts) {
    const verdicts = [];
    if (!iso) {
    ...
    const skippedTree = (iso.skipped ?? []).some((s) => s.id === 'structuralDiff' || s.id === 'iso.tree' || s.id === 'tree');
