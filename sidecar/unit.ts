...
function testRowHashPrimitives(): void {
  ...
  assert.notStrictEqual(hashProp(1, 'x'), hashAttr('value', 'x'), 'hashProp must not collide with hashAttr for the same text');
  assert.notStrictEqual(hashProp(1, 'x'), hashProp(2, 'x'), 'propId must affect hashProp');
  assert.notStrictEqual(hashProp(1, true), hashProp(1, false), 'bool prop values must not collide');
...
function testPropSetWire(): void {
  ...
  const valueOp: FrameOp = { op: OpCode.PropSet, node: 10, propId: PROP_ID_VALUE, value: 'Ada' };
  const checkedOp: FrameOp = { op: OpCode.PropSet, node: 11, propId: PROP_ID_CHECKED, value: true };
  const selectedOp: FrameOp = { op: OpCode.PropSet, node: 12, propId: PROP_ID_SELECTED, value: false };

  const valueBytes = new BinaryFrameEncoder().encode(createFrame({ generation: 1, sequence: 1, ops: [valueOp] }))[0]!;
  ...
  assert.strictEqual(valueGot.node, 10);
  assert.strictEqual(valueGot.propId, PROP_ID_VALUE);
  assert.strictEqual(valueGot.value, 'Ada');

  const checkedBytes = new BinaryFrameEncoder().encode(createFrame({ generation: 1, sequence: 1, ops: [checkedOp] }))[0]!;
  const checkedDecoded = decodeFramePart(checkedBytes, new PersistentStringTable());
  assert.ok(checkedDecoded.ok, 'CHECKED PROP_SET must decode');
  if (!checkedDecoded.ok) return;
  const checkedGot = checkedDecoded.part.ops[0];
  assert.strictEqual(checkedGot?.op, OpCode.PropSet);
  if (checkedGot?.op !== OpCode.PropSet) return;
  assert.strictEqual(checkedGot.propId, PROP_ID_CHECKED);
  assert.strictEqual(checkedGot.value, true);

  const selectedBytes = new BinaryFrameEncoder().encode(createFrame({ generation: 1, sequence: 1, ops: [selectedOp] }))[0]!;
  const selectedDecoded = decodeFramePart(selectedBytes, new PersistentStringTable());
  assert.ok(selectedDecoded.ok, 'SELECTED PROP_SET must decode');
  if (!selectedDecoded.ok) return;
  const selectedGot = selectedDecoded.part.ops[0];
  assert.strictEqual(selectedGot?.op, OpCode.PropSet);
  if (selectedGot?.op !== OpCode.PropSet) return;
  assert.strictEqual(selectedGot.propId, PROP_ID_SELECTED);
  assert.strictEqual(selectedGot.value, false);

  const badId = decodeFramePart(patchPropSetPropId(valueBytes, 0x0b), new PersistentStringTable());
  assert.strictEqual(badId.ok, false);
  if (!badId.ok) assert.strictEqual(badId.reason, 'malformed');

  const badBool = decodeFramePart(patchPropSetBool(checkedBytes, 2), new PersistentStringTable());
  assert.strictEqual(badBool.ok, false);
  if (!badBool.ok) assert.strictEqual(badBool.reason, 'malformed');
  ...
  assert.throws(() =>
    new BinaryFrameEncoder().encode(
      createFrame({
        generation: 1,
        sequence: 1,
        ops: [{ op: OpCode.PropSet, node: 10, propId: 0x0b, value: 'x' }],
      }),
    ),
  );
...
function patchPropSetPropId(bytes: Uint8Array, propId: number): Uint8Array {
  const out = bytes.slice();
  const o = firstOpOffset(out);
  assert.strictEqual(out[o], OpCode.PropSet);
  out[o + 1 + 4] = propId;
  return out;
}

function patchPropSetBool(bytes: Uint8Array, flag: number): Uint8Array {
  const out = bytes.slice();
  const o = firstOpOffset(out);
  assert.strictEqual(out[o], OpCode.PropSet);
  out[o + 1 + 4 + 1] = flag;
...
function testPropSetTableAndCheck(): void {
  const table = new ReplicatedTable();
  table.createElementRow(10, 'input', [{ name: 'value', value: '' }]);
  const afterAttr = table.getRow(10)!.contentHash;
  assert.strictEqual(table.getProp(10, PROP_ID_VALUE), undefined);

  applyOpToTable(table, { op: OpCode.PropSet, node: 10, propId: PROP_ID_VALUE, value: 'Ada' });
  assert.strictEqual(table.getProp(10, PROP_ID_VALUE), 'Ada');
  assert.notStrictEqual(table.getRow(10)!.contentHash, afterAttr, 'PROP_SET must change contentHash independently of the value attribute');

  const reference = new ReplicatedTable();
  reference.createElementRow(10, 'input', [{ name: 'value', value: '' }]);
  applyOpToTable(reference, { op: OpCode.PropSet, node: 10, propId: PROP_ID_VALUE, value: 'Ada' });

  const client = new ReplicatedTable();
  client.createElementRow(10, 'input', [{ name: 'value', value: '' }]);
  const result = applyFrameToTableChecked(client, false, [
    { op: OpCode.PropSet, node: 10, propId: PROP_ID_VALUE, value: 'Ada' },
    { op: OpCode.Check, scope: CHECK_SCOPE_TABLE, lo: 0, hi: 0, hash: reference.tableHash },
  ]);
  assert.strictEqual(result.ok, true, 'CHECK after PROP_SET must pass from table hash, not live DOM');
  assert.strictEqual(client.getProp(10, PROP_ID_VALUE), 'Ada');
...
function testFormPropDirtyDoesNotBlockTable(): void {
  const dirty = new FormPropDirty();
  dirty.mark(10);
  assert.strictEqual(dirty.isDirty(10), true);

  const table = new ReplicatedTable();
  table.createElementRow(10, 'input', []);
  const ops: FrameOp[] = [
    { op: OpCode.PropSet, node: 10, propId: PROP_ID_VALUE, value: 'o' },
  ];
  applyOpsToTable(table, ops);
  assert.strictEqual(table.getProp(10, PROP_ID_VALUE), 'o');

  const withCheck: FrameOp[] = [
    ...ops,
    { op: OpCode.Check, scope: CHECK_SCOPE_TABLE, lo: 0, hi: 0, hash: table.tableHash },
  ];
  const other = new ReplicatedTable();
  other.createElementRow(10, 'input', []);
  const result = applyFrameToTableChecked(other, false, withCheck);
  assert.strictEqual(result.ok, true, 'CHECK must not look at a dirty live field');
  assert.strictEqual(dirty.isDirty(10), true, 'phase 1 must not clear dirty');

  dirty.hold({ op: OpCode.PropSet, node: 10, propId: PROP_ID_VALUE, value: 'oi' });
  const held = dirty.take(10);
  assert.strictEqual(held?.value, 'oi');

  const mismatch = formControlSnapsEqual([{ key: 'name', value: 'Ada-1' }], [{ key: 'name', value: 'Ada' }]);
  assert.strictEqual(mismatch.identical, false);
  const match = formControlSnapsEqual([{ key: 'name', value: 'Ada-1' }], [{ key: 'name', value: 'Ada-1' }]);
  assert.strictEqual(match.identical, true);
 ...
 */
function testApplyFrameToTableCheckedPhase1Pres(): void {
  {
    const table = new ReplicatedTable();
    applyOpsToTable(table, [
    ...
    assert.strictEqual(result.ok, false);
    if (!result.ok && result.opName !== 'check') {
      assert.strictEqual(result.opName, 'insert');
      assert.strictEqual(result.reason, 'precondition');
    } else assert.fail(JSON.stringify(result));
    assert.strictEqual(table.getRow(2)!.parent, 0, 'failed INSERT must not attach');
  }
  {
    const table = new ReplicatedTable();
    applyOpsToTable(table, [
    ...
    const result = applyFrameToTableChecked(table, false, [
      { op: OpCode.Remove, parent: 1, ids: [3] },
    ]);
    assert.strictEqual(result.ok, false);
    if (!result.ok && result.opName !== 'check') {
      assert.strictEqual(result.opName, 'remove');
      assert.strictEqual(result.reason, 'precondition');
    } else assert.fail(JSON.stringify(result));
    assert.strictEqual(table.getRow(3)!.parent, 2, 'failed REMOVE must not detach');
  }
  {
    const table = new ReplicatedTable();
    applyOpsToTable(table, [
      { op: OpCode.NodeNew, id: 2, kind: NodeKind.Text, value: 't' },
    ]);
    const result = applyFrameToTableChecked(table, false, [
      { op: OpCode.AttrSet, node: 2, attrs: [{ name: 'id', value: 'x' }] },
    ]);
    assert.strictEqual(result.ok, false);
    if (!result.ok && result.opName !== 'check') {
      assert.strictEqual(result.opName, 'attrSet');
      assert.strictEqual(result.reason, 'precondition');
    } else assert.fail(JSON.stringify(result));
  }
  {
    ...
    const result = applyFrameToTableChecked(table, false, [
      { op: OpCode.PropSet, node: 2, propId: PROP_ID_VALUE, value: 'nope' },
    ]);
    assert.strictEqual(result.ok, false);
    if (!result.ok && result.opName !== 'check') {
      assert.strictEqual(result.opName, 'propSet');
      assert.strictEqual(result.reason, 'precondition');
    } else assert.fail(JSON.stringify(result));
  }
  {
    ...
    assert.strictEqual(result.ok, false);
    if (!result.ok && result.opName !== 'check') {
      assert.strictEqual(result.opName, 'textSet');
      assert.strictEqual(result.reason, 'precondition');
    } else assert.fail(JSON.stringify(result));
  }
  {
    ...
    assert.strictEqual(result.ok, false);
    if (!result.ok && result.opName !== 'check') {
      assert.strictEqual(result.opName, 'ruleSet');
      assert.strictEqual(result.reason, 'precondition');
    } else assert.fail(JSON.stringify(result));
  }
  {
    ...
    assert.strictEqual(result.ok, false);
    if (!result.ok && result.opName !== 'check') {
      assert.strictEqual(result.opName, 'sheetNew');
      assert.strictEqual(result.reason, 'precondition');
    } else assert.fail(JSON.stringify(result));
    assert.strictEqual(table.has(50), false, 'failed SHEET_NEW must not create a row');
  }
  {
    const table = new ReplicatedTable();
    applyOpsToTable(table, [
      { op: OpCode.SheetNew, id: 50, scope: CSSOM_SCOPE_MAIN, hostNode: 0, before: INSERT_AT_END },
    ]);
    const result = applyFrameToTableChecked(table, false, [
      { op: OpCode.RuleNew, sheet: 50, id: 51, before: 999, text: 'a{}' },
    ]);
    assert.strictEqual(result.ok, false);
    if (!result.ok && result.opName !== 'check') {
      assert.strictEqual(result.opName, 'ruleNew');
      assert.strictEqual(result.reason, 'precondition');
    } else assert.fail(JSON.stringify(result));
    assert.strictEqual(table.has(51), false);
  }
  {
    const table = new ReplicatedTable();
    applyOpsToTable(table, [
    ...
    const result = applyFrameToTableChecked(table, false, [
      { op: OpCode.Insert, parent: 1, before: 0, ids },
    ]);
    assert.strictEqual(result.ok, false);
    if (!result.ok && result.opName !== 'check') {
      assert.strictEqual(result.opName, 'insert');
  ...
  }
  {
    ...
    assert.strictEqual(table.getRow(2)!.parent, 1);
  }
...
function testDomFrameApplierFlushStopsOnDesync(): void {
  ...
  assert.strictEqual(result.lastIndex, 0);
  assert.strictEqual(
    applied.length,
    0,
    'PP-APPLY-1: no apply of later batch frames after mid-batch desync',
  );

  const appliedOk: number[] = [];
  const allOk = applyFramesUntilDesync([{ sequence: 10 }, { sequence: 11 }], (frame) => {
    appliedOk.push(frame.sequence);
    return true;
  });
  assert.strictEqual(allOk.stoppedEarly, false);
  assert.deepStrictEqual(appliedOk, [10, 11]);
...
function testApplyAttrPairsReportsFailure(): void {
  const calls: Array<{ name: string; value: string }> = [];
  assert.strictEqual(
    applyAttrPairs((name, value) => {
  ...
  assert.strictEqual(calls[0]!.name, 'id');
  assert.strictEqual(calls[1]!.name, 'class');

  const partial: string[] = [];
  assert.strictEqual(
    applyAttrPairs((name, value) => {
      if (name === 'bad') throw new Error('InvalidCharacterError');
    ...
    false,
    'PP-APPLY-2: throw from setAttribute → false (callers desync)',
  );
  assert.strictEqual(partial.length, 1);
  assert.strictEqual(partial[0], 'ok=1');
...
function testPlanRuleSetApplySealScope(): void {
  assert.strictEqual(planRuleSetApply(true).mode, 'styleDeclarations');
  assert.strictEqual(planRuleSetApply(false).mode, 'desync');
  console.log('[unit] planRuleSetApply CSSStyleRule-only seal (PP-CSSOM-A-1) ok');
}

function testLabBlueprintValidateCycleAndParallelSnap(): void {
  const cyclic: LabBlueprint = {
    id: 'cyclic',
    description: 'x',
    sessionPolicy: 'cold',
    queues: [
  ...
  const c = validateBlueprint(cyclic);
  assert.strictEqual(c.ok, false);

  const parallelSnap: LabBlueprint = {
    id: 'par',
    description: 'x',
    sessionPolicy: 'cold',
    queues: [
  ...
  const p = validateBlueprint(parallelSnap);
  assert.strictEqual(p.ok, false);
  console.log('[unit] lab blueprint validate cycle + parallel snap ok');
}

async function testLabBlueprintScheduleDependsAndAwaits(): Promise<void> {
  const order: string[] = [];
  const bp: LabBlueprint = {
    id: 'sched',
    description: 'x',
    sessionPolicy: 'cold',
    queues: [
  ...
  const r = await runBlueprintSchedule(bp, {
  ...
  assert.ok(order.indexOf('boot') < order.indexOf('work'));
  assert.ok(order.indexOf('boot') < order.indexOf('side'));
  assert.ok(order.indexOf('fold') > order.indexOf('work'));
  assert.ok(order.indexOf('fold') > order.indexOf('side'));
...
function testCssomGroupingContentChangeEmitsDropNew(): void {
  class CSSMediaRule {}
  class CSSStyleRule {}
  assert.strictEqual(ruleAcceptsInPlaceSet(new CSSStyleRule()), true);
  assert.strictEqual(ruleAcceptsInPlaceSet(new CSSMediaRule()), false);
  ...
  assert.strictEqual(table.getRow(oldId)?.kind, NodeKind.Rule);
  assert.strictEqual(table.getRow(oldId)?.contentHash, hashValue('@media (max-width: 1px){.a{color:red}}'));
  ...
  assert.ok(!live.some((op) => op.op === OpCode.RuleSet), 'grouping content change must not RULE_SET');
  assert.ok(live.some((op) => op.op === OpCode.RuleDrop));
  assert.ok(live.some((op) => op.op === OpCode.RuleNew));
  const newId = ids.peekRule(media)!;
  assert.notStrictEqual(newId, oldId, 'replace allocates a new id');

  applyOpsToTable(table, live);
  assert.strictEqual(table.getRow(oldId), undefined, 'old rule id gone after DROP');
  assert.strictEqual(table.getRow(newId)?.kind, NodeKind.Rule);
  assert.strictEqual(table.getRow(newId)?.contentHash, hashValue(nextText));
  assert.deepStrictEqual(table.orderedChildIds(sheetId), [newId]);
...
function testCssomApplyIndex(): void {
  ...
  assert.strictEqual(declarationBlockFromRuleText('.app{background:#161310;color:#f3ead8}'), 'background:#161310;color:#f3ead8');
  assert.strictEqual(declarationBlockFromRuleText('color:red'), 'color:red');
  console.log('[unit] cssom apply index ok');
}

/** PP-CSSOM-A-3 / SEAL-CSSOM-P0-EOF: end-of-frame sheet+rule membership/order. */
function testCssomEndOfFrameMatch(): void {
  const sheet = 100;
  const r1 = 101;
  const r2 = 102;
  const table = new ReplicatedTable();
  applyOpsToTable(table, [
    { op: OpCode.SheetNew, id: sheet, scope: CSSOM_SCOPE_MAIN, hostNode: 0, before: INSERT_AT_END },
    { op: OpCode.RuleNew, sheet, id: r1, before: INSERT_AT_END, text: '.a{}' },
    { op: OpCode.RuleNew, sheet, id: r2, before: INSERT_AT_END, text: '.b{}' },
  ]);
  assert.deepStrictEqual(orderedRuleIds(table, sheet), [r1, r2]);

  const tableSheets = orderedSheetIds(table);
  const tableRules = new Map<number, readonly number[]>([[sheet, orderedRuleIds(table, sheet)]]);
  const ok = matchCssomEndOfFrame(
    tableSheets,
    tableRules,
    new Set([sheet]),
    new Map([[sheet, [r1, r2]]]),
  ...
  const missSheet = matchCssomEndOfFrame(tableSheets, tableRules, new Set(), new Map());
  assert.strictEqual(missSheet.ok, false);
  if (!missSheet.ok) {
    assert.strictEqual(missSheet.op, 'sheetNew');
    assert.strictEqual(missSheet.id, sheet);
  }

  const missRule = matchCssomEndOfFrame(
    tableSheets,
    tableRules,
    new Set([sheet]),
    new Map([[sheet, [r1]]]),
  );
  assert.strictEqual(missRule.ok, false);
  if (!missRule.ok) {
    assert.strictEqual(missRule.op, 'ruleNew');
    assert.strictEqual(missRule.id, r2);
  }

  const badOrder = matchCssomEndOfFrame(
    tableSheets,
    tableRules,
    new Set([sheet]),
    new Map([[sheet, [r2, r1]]]),
  );
  assert.strictEqual(badOrder.ok, false);
  if (!badOrder.ok) {
    assert.strictEqual(badOrder.op, 'ruleOrder');
    assert.strictEqual(badOrder.id, r1);
  }
