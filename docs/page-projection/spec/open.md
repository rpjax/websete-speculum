...
### BUG — Virtual mint-without-drop census hang (**OPEN**, scoped to frozen `os-abs` since 2026-08-27)

| Id | Symptom | Notes |
|----|---------|-------|
| **PP-INPUT-VIRTUAL-MINT-GHOST** | Real site (eneba.com) with ad/tracker iframe churn: clicks silently die intermittently. Console: `input_reject apply_scroll_failed:invoke idle timeout (2000ms) virtual_apply`, repeating. `probes/input-pipeline.json` reports `dropped: 0` for the **same** run — dossier is blind to this class of drop. | Root cause on the **Virtual** (root/producer) side, not Projected, not the input adapter (reproduces on `os-abs`, unrelated to `sparse-cdp`). `VirtualDomainBus.isDeliverableDestination` is wired to `ContextIdMint.hasMinted(id)` (`rootRuntime.ts`), which is monotonic — once an id is minted it is "deliverable" forever, even after its iframe is removed from the DOM. A short-lived ad iframe mints a contextId, dies in <1s; if a click's S6 census still names that id (Projected's own registry can itself lag the RPC round trip — this is a race, not just a stale-flag bug), `routeOutbound` finds no live child window for it but `isDeliverableDestination` still says yes, so it skips the fail-closed path (only reachable for **never**-minted ids) and falls through to a hopeful broadcast that nobody answers. `bus.requestApplyScroll` then blocks the full `RESUME_TIMEOUT_MS` (2000ms); `applyScrollCensus`'s `Promise.all` over `census.contexts` waits on it before resolving **any** context (including the still-live root ctx 1); that ties or loses the race against the sidecar's own `LOOPBACK_INVOKE_IDLE_MS` (2000ms) loopback-invoke timeout, so the sidecar gets an opaque idle-timeout instead of Virtual's real (slower) `{ok:false}` and rejects the whole down/up pair. A `ContextRegistry` class (`admitHost`/`dropHost` → `contextHostAdmitted`/`contextHostDropped` events) already exists and is exercised on the **Projected** side (`lab/static/client.js`) for its own bookkeeping, but on the **Virtual/root** side it is only used to `announceRootOnline` — nothing subscribes to host-admitted/dropped there to keep `isDeliverableDestination` (or a replacement live-peer set) current. Already flagged as distinct from PP-INPUT-NESTED-DROP-LOAD in that bug's own note ("Virtual mint-without-dropHost (wire ghosts in census `[1,N]`)") — this is that bug, now confirmed live on a real site. **Secondary defect, same finding:** `probes/input-pipeline.json`'s `dropped`/`dropsByError` counters are not wired to `EventApplier`'s `onReject`/the `input_reject` console path at all — a run can show `dropped: 0` while real console rejections happened, so the dossier cannot be trusted alone to prove input health; must cross-check console/telemetry. Repro: real site with iframe/ad churn (`eneba.com`); `journal/contexts.json` in the dossier shows short-lived `frameCount:1` contexts correlating with the failure window. **Do not** patch by shortening `RESUME_TIMEOUT_MS`/`LOOPBACK_INVOKE_IDLE_MS` or by skipping census entries with unknown ids as a band-aid — that is exactly the kind of ad-hoc workaround this file bans; the fix is a real live/attached-peer registry backing `isDeliverableDestination`, and honest drop counting in the dossier. **Update 2026-08-27 — no longer a cutover-blocker, still open:** `os-abs` is now frozen legacy, opt-in only (decision-log.md 2026-08-27); `sparse-cdp` is the canonical default and discards S6 census for clicks entirely, resolving by nodeId through a single targeted `resolveNodeHit` call (not a `Promise.all` fan-out over every known context) — the canonical path cannot hit this class of hang at all, not just "narrower blast radius." This bug stays **OPEN** for `os-abs` itself: `isDeliverableDestination` is still wrong on the Virtual side, and anyone opting into `os-abs` hits it unchanged. Not fixed, not closed — just no longer blocking the default. |
