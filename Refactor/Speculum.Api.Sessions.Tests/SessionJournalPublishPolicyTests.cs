...
public sealed class SessionJournalPublishPolicyTests
{
    ...
    public void SessionAndProfileLifecycleFacts_AreGuaranteed()
    {
        ...
        Assert.True(catalog.TryGet<PageProjectionDiffOutputStreamOpened>(out var outputStreamOpened));
        Assert.Equal(PublishPolicy.BestEffort, outputStreamOpened.PublishPolicy);
        Assert.Equal(1, outputStreamOpened.SchemaVersion);
        Assert.False(catalog.IsTypeEnabled("Telemetry.Sessions.PageProjection.Diff.OutputStreamOpened"));
        Assert.True(catalog.TryGet<PageProjectionDiffOutputStreamClosed>(out var outputStreamClosed));
        Assert.Equal(PublishPolicy.BestEffort, outputStreamClosed.PublishPolicy);
        Assert.Equal(1, outputStreamClosed.SchemaVersion);
        Assert.False(catalog.IsTypeEnabled("Telemetry.Sessions.PageProjection.Diff.OutputStreamClosed"));
        Assert.True(catalog.TryGet<PageProjectionDiffFanOutEnqueued>(out var fanOutEnqueued));
        Assert.Equal(PublishPolicy.BestEffort, fanOutEnqueued.PublishPolicy);
        Assert.Equal(3, fanOutEnqueued.SchemaVersion);
        Assert.False(catalog.IsTypeEnabled("Telemetry.Sessions.PageProjection.Diff.FanOutEnqueued"));
        Assert.True(catalog.TryGet<PageProjectionDiffStreamDequeued>(out var streamDequeued));
        Assert.Equal(PublishPolicy.BestEffort, streamDequeued.PublishPolicy);
        Assert.Equal(3, streamDequeued.SchemaVersion);
        Assert.False(catalog.IsTypeEnabled("Telemetry.Sessions.PageProjection.Diff.StreamDequeued"));
