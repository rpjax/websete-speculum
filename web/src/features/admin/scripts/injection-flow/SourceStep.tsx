...
export function SourceStep({ draft, onChange, returnUrl }: { draft: Injection; onChange: (next: Injection) => void; returnUrl: string }) {
  ...
  return <div className="space-y-5"><div><h2 className="text-lg font-semibold">Source</h2><p className="text-sm text-muted-foreground">Choose the script to attach at session start.</p></div>
    ...
    {source.sourceType === 'stored' ? <div className="space-y-2"><Label>Stored script</Label>{scripts.length ? <Select value={source.storedScriptId ?? ''} onValueChange={(storedScriptId) => onChange({ ...draft, source: { ...source, storedScriptId } })}><SelectTrigger><SelectValue placeholder="Pick from library" /></SelectTrigger><SelectContent>{scripts.map((script) => <SelectItem key={script.id} value={script.id}>{script.name}</SelectItem>)}</SelectContent></Select> : <HelperCallout title="Library is empty" action={{ label: 'Upload first', href: `/w7s/admin/scripts/upload?returnUrl=${encodeURIComponent(returnUrl)}` }}>Upload a .js file before using it as a stored source.</HelperCallout>}<p className="text-sm text-muted-foreground">Content is snapshotted from the library at session start.</p></div>
