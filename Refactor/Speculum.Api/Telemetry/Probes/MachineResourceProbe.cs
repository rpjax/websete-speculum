...
/// Reads configurable procfs (<see cref="HostTelemetryConfiguration.ProcPath"/>). On Windows or
...
public sealed class MachineResourceProbe
{
    ...
    public HostTelemetry Sample(HostTelemetryConfiguration options)
    {
    ...
    private HostTelemetry CollectFresh(HostTelemetryConfiguration options, DateTimeOffset now)
    {
    ...
    private static HostTelemetry Unavailable(string hostname, HostTelemetryConfiguration options)
    {
        var (diskFree, diskTotal) = ReadDisk(options.DiskPath);
        return new HostTelemetry(
            Hostname: hostname,
    ...
    private static HostTelemetry ApplyIncludes(HostTelemetry full, HostTelemetryConfiguration options)
        => full with
        {
