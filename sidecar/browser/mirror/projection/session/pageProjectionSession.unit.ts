...
async function waitDataPlaneOpen(
  ...
  while (Date.now() < deadline) {
    try {
      const status = await session.probeLoopbackStatus();
      if (!status.nodeEstablished) {
        lastErr = `loopback not established (gen=${status.generation})`;
        await wait(100);
        continue;
      }
      // Prefer Virtual MAIN-world / RPC proof; node-only is not enough for accept, but after
      // hard nav the CDP execution context can lag — loopback RPC is the wire truth.
      if (status.virtualEstablished) {
        ...
        if (
          lastErr &&
          !/data plane not open|not_open|not_established|producer_booting|no invoke handler/i.test(
            lastErr,
          )
        ) {
        ...
        lastErr =
          r.error ??
          `loopback desync node=${status.nodeEstablished} virtual=${status.virtualEstablished} gen=${status.generation}`;
      }
    } catch (err) {
...
<pre id="pad">${pad}</pre>
  ...
  try {
    ...
    const coldPlane = await waitDataPlaneOpen(session, 30_000);
