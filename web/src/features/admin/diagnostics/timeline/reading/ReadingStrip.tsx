...
export function ReadingStrip({
  ...
  const statsLabel = [
    `${stats.beats} ${stats.beats === 1 ? 'beat' : 'beats'}`,
    `${stats.lanes} ${stats.lanes === 1 ? 'lane' : 'lanes'}`,
    `${stats.chapters} ${stats.chapters === 1 ? 'chapter' : 'chapters'}`,
  ].join(' · ')
  const filterActive =
      ...
      <div className="flex flex-col gap-1.5 px-2.5 py-2">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <div className="ml-auto flex shrink-0 items-center gap-1.5">
            ...
            <span
              className="hidden tabular-nums text-[10px] text-muted-foreground xs:inline sm:inline"
              title={statsLabel}
            >
              {statsLabel}
            </span>
              ...
              {menuOpen && (
                <div className="absolute right-0 z-30 mt-1 w-56 rounded-md border border-border bg-card p-1 shadow-md">
                  ...
                  <p className="px-3 pb-2 text-[10px] text-muted-foreground/80">
                    Opens Investigate with the current period. Tools stay independent.
                  </p>
                </div>
              )}
            </div>
