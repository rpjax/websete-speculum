...
import {
  AlertTriangle, CalendarRange, Plus, X,
...
interface TelemetryMonitorChartProps {
  samples: ResourceSample[]
  metrics: MetricDef[]
  onBrushRange?: (from: number, to: number) => void
  defaultEnabled?: string[]
  /** Externally-driven crosshair time (e.g. jumped-to from Monitor hints). */
...
export function TelemetryMonitorChart({
  ...
  const [internalEnabled, setInternalEnabled] = useState<Set<string>>(
    new Set(initialEnabled.length ? initialEnabled : [metrics[0]?.key]),
  )
  const enabled = useMemo(
    () => (enabledKeys ? new Set(enabledKeys) : internalEnabled),
    [enabledKeys, internalEnabled],
  )
  function setEnabled(next: Set<string> | ((prev: Set<string>) => Set<string>)) {
    const resolved = typeof next === 'function' ? next(enabled) : next
    if (onEnabledKeysChange) onEnabledKeysChange([...resolved])
    else setInternalEnabled(resolved)
  }
  ...
  const metricStats = useMemo(
    () => activeMetrics.map((m) => ({ metric: m, stats: seriesStats(chartSamples, m) })),
    [activeMetrics, chartSamples],
  )
  ...
  const H = tall ? 420 : 320
  const chartGeom = useMemo(() => ({ W: 720, H, pad: { l: 52, r: 52, t: 20, b: 28 } }), [H])
        ...
        <div className="px-2 py-1 relative">
          {viewMode === 'stacked' ? (
            ...
            <svg ref={svgRef} viewBox={`0 0 ${chartGeom.W} ${chartGeom.H}`} className="w-full select-none touch-none"
              ...
              {/* State bands behind series */}
              {stateWindows.map((w, wi) => {
                const x1 = xAt(w.startIndex)
                const x2 = xAt(w.endIndex)
                return (
                  <rect key={wi} x={x1} y={chartGeom.pad.t} width={Math.max(2, x2 - x1)} height={plotH}
                    className={w.kind === 'degraded' ? 'fill-red-500/15' : 'fill-amber-500/15'} />
                )
              })}
              {[0, 0.25, 0.5, 0.75, 1].map((f) => (
                <line key={f} x1={chartGeom.pad.l} x2={chartGeom.W - chartGeom.pad.r}
                  ...
                  className="stroke-border/20" strokeWidth={0.75} />
              ))}
              {[0, 0.25, 0.5, 0.75, 1].map((f) => {
                const idx = Math.round(f * (chartSamples.length - 1))
                return (
                  <text key={f} x={xAt(idx)} y={chartGeom.H - 6} textAnchor="middle" className="fill-muted-foreground text-[10px]">
                    {new Date(timestamps[idx]).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </text>
                )
              })}
              {[...divergencePoints].map((i) => (
                <circle key={`div-${i}`} cx={xAt(i)} cy={chartGeom.pad.t + 8} r={3} className="fill-amber-400/80" />
              ))}
              {seriesData.map((s, si) => {
              ...
              {activeIdx != null && activeIdx >= 0 && activeIdx < chartSamples.length && (
                <>
                  <line x1={xAt(activeIdx)} x2={xAt(activeIdx)} y1={chartGeom.pad.t} y2={chartGeom.pad.t + plotH}
                    className="stroke-foreground/30" strokeWidth={1.25} strokeDasharray="3 2" />
                  {/* Floating tooltip near crosshair */}
                  <g transform={`translate(${Math.min(xAt(activeIdx) + 8, chartGeom.W - 130)}, ${chartGeom.pad.t + 8})`}>
                    <rect width={122} height={14 + activeMetrics.length * 14} rx={3} className="fill-card stroke-border" strokeWidth={1} />
          ...
          {(viewMode === 'overlay' || viewMode === 'stacked') && (
            <p className="px-1 pb-1 text-[10px] text-muted-foreground/50">
              {SCALE_OPTIONS.find((o) => o.value === scaleMode)?.hint}
              {divergencePoints.size > 0 && (
                <span className="ml-2 text-amber-400/80">● {divergencePoints.size} divergence{divergencePoints.size > 1 ? 's' : ''}</span>
              )}
...
function StatsInspector({ metricStats, chartSamples }: {
  metricStats: { metric: MetricDef; stats: ReturnType<typeof seriesStats> }[]
  chartSamples: ResourceSample[]
}) {
  return (
    <div className="space-y-2">
      <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Range stats</p>
        ...
        return (
          <div key={metric.key} className="rounded border border-border/30 bg-muted/5 px-2 py-1.5">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] font-medium truncate">{metric.label}</span>
              {path && (
                <svg viewBox="0 0 56 16" className="ml-auto h-4 w-14 shrink-0">
                  <path d={path} fill="none" stroke={metric.color} strokeWidth={1.2} />
                </svg>
              )}
            </div>
            <div className="mt-1 grid grid-cols-4 gap-1 text-center">
              {([['min', stats.min], ['avg', stats.avg], ['max', stats.max], ['last', stats.last]] as const).map(([k, v]) => (
                <div key={k}>
                  <p className="text-[8px] uppercase text-muted-foreground/50">{k}</p>
                  <p className="text-[11px] font-bold tabular-nums">{round1(v)}{metric.unit}</p>
                </div>
              ))}
            </div>
          </div>
