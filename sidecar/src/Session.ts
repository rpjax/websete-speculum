    ...
    async handleMessage(raw: string): Promise<void> {
        ...
        try {
            switch (msg.type) {
                ...
                case 'goback':
                    await this._runNavigation(() =>
                        this._page.goBack({ waitUntil: 'domcontentloaded', timeout: 30_000 }),
                    );
                    break;

                case 'goforward':
                    await this._runNavigation(() =>
                        this._page.goForward({ waitUntil: 'domcontentloaded', timeout: 30_000 }),
                    );
                    break;
    ...
    private async _runNavigation(action: () => Promise<unknown>): Promise<void> {
        ...
        try {
            ...
            if (this._navigation.isCurrent(generation)) {
                console.warn(
                    `[${this.sessionId}] Navigation error:`,
