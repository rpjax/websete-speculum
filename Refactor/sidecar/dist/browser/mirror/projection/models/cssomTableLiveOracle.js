...
function compareTableToLiveCssom(table, liveSheets) {
    ...
    const tableSheets = orderedKindChildIds(table, frame_1.DOCUMENT_ID, opcodes_1.NodeKind.Sheet);
    const liveSheetIds = liveSheets.map((s) => s.id);
    if (!idsEqual(tableSheets, liveSheetIds)) {
        record('#sheets', 'sheet_order_mismatch', `table=[${tableSheets.slice(0, 8).join(',')}] live=[${liveSheetIds.slice(0, 8).join(',')}]`);
    }
