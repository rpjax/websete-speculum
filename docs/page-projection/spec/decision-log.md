...
|------|-------|------------------------|
| 2026-08-30 | **K5 = CSP meta, not iframe `sandbox` (Rodrigo / iOS)** — WebKit/iOS does not deliver touch to sandboxed iframes (`allow-same-origin` without `allow-scripts`). K5 stays: no page JS on Projected. Replaced `iframe.sandbox` with `Content-Security-Policy: script-src 'none'; object-src 'none'` in `PROJECTED_STANDARDS_SRCDOC` + `ensureProjectedK5Csp` before mirrored `<script>` materialize. Submit / navigable-link guards remain on `attachProjectedNativeGuard`. | `projectedBlankIframe.ts` · `surface.ts` · `nestedResyncSurface.ts` · `applyDom.ts` · `projectedK5.unit.ts` · [motor-0.3.0.md](motor-0.3.0.md) |
| 2026-08-29 | **B1 per-session extension dir + kill managedTabId (Rodrigo)** — C2 endpoint isolated via `materializeSpeculumPpForSession` (temp copy + delete on stop). `managedTabId` deleted (1 session = 1 tab; single-tab only). Dead `webgl-spoof` / `speculum-plane` dirs removed; draft inject md deleted. | [runtime-redesign.md](runtime-redesign.md) §0 #3/#7 · §15 |
| 2026-08-29 | **Runtime redesign cutover in tree** — unified `sidecar/extensions/speculum-pp/` (webgl + plane + Virtual MAIN); C2 SessionConfig/ACK before navigate; `awaitProjectionConfig` + `initContext` (SW generation for root); MessagePort ContextBus; mint 1/RPC hold-frame; `EPOCH_RESET` removed (Projected teardown by generation); CDP installer/lateBoot deleted from happy path. P0 OOPIF catch retained on CSP frame CDP. Stealth (V3) still to measure on real antibot before accept. | [runtime-redesign.md](runtime-redesign.md) · [browser-session.md](browser-session.md) · [frame-protocol.md](frame-protocol.md) §4.1 |
| 2026-08-29 | **Runtime carrier redesign SEALED (Rodrigo)** — CDP inject / lateBoot superseded. Delivery = one static MV3 extension (MAIN + `all_frames` + `match_origin_as_fallback`); **C2** session config via SW (ACK before navigate, `T_config=2000`, fail-closed). ContextBus = **MessagePort** (no broadcast). Identity = **`initContext`** → `{ contextId, generation }` (not “handshake”). Mint = **1 id / RPC**; pending host → **no frame emit that tick**. Timeouts: nested dormant 2000 ms, root→SW crash 5000 ms. One tab/session; bfcache → re-`initContext`. Stealth spike before cutover; thin bootstrap out of V1. P0 OOPIF catch stays decoupled. | [runtime-redesign.md](runtime-redesign.md) §0 |
| 2026-08-28 | **`sparse-cdp` click = local % in element box (Rodrigo)** — Projected sends `localX`/`localY` ∈ [0,1] (top-left origin of `getBoundingClientRect`); Virtual maps onto the live node rect for CDP. Absolute client CSS is journal stamp only — **not** a hit criterion (no `point_outside_node`). Survives Projected≠Virtual layout (phone vs desktop, margins). Omit local → element center (lab). No compat shim for absolute-as-hit. | [input.md](input.md) · `localHit.ts` · `projectedInputCapture.ts` · `bootstrap.ts` (`resolveNodeHit`) · `EventApplier.ts` · `clickDelivery.ts` |
| 2026-08-28 | **Console relay via CDP Runtime** — Patchright `page.on('console')` / `pageerror` deliver zero events; lab dossier `telemetry/console.ndjson` was blind to Virtual `console.*`. Fix: `attachCdpConsoleRelay` (`Runtime.enable` + `consoleAPICalled` / `exceptionThrown`) on PP + Video `Evaluate`. | `cdpConsoleRelay.ts` · `PageProjectionBrowserSession.ts` · `Evaluate.ts` |
| 2026-08-29 | **K4 projected blank = srcdoc at birth (delete open/write)** — `document.open`/`write` orphaned nested-in-nested Documents (`defaultView` null → `createDocumentType` null). One bootstrap: stamp `PROJECTED_STANDARDS_SRCDOC` **before** insert (NODE_NEW nested host, root surface, resync standby), wait load, strip skeleton. Deleted `standardsBlankDocument.ts`. | `projectedBlankIframe.ts` · `surface.ts` · `nestedResyncSurface.ts` · `applyDom.ts` · `ProjectionClient.ts` · [frame-protocol.md](frame-protocol.md) §1.2 |
| 2026-08-28 | **PP drops Xorg/`Display` entirely** — sparse-cdp never starts `DisplayAllocator`/`Display.start`; `launchChrome` headed = native desktop / ambient host `DISPLAY` only. VideoStreaming OS path keeps Xorg+uinput. | `PageProjectionBrowserSession.ts` · `ChromeRuntime.ts` · [input.md](input.md) · `sidecar/LAB-DOCKER.md` |
| 2026-08-28 | **Close sidecar Now items 1/2/4** — (1) **PP-EXTENSION-PLANE** EP-15 accept: extension-only carrier + zero page LNA on lab/Superbet/Eneba dossiers + units. (2) Virtual assets V1 proven: `lab-assets-stress.js` 4/4 (fixture + Superbet + Eneba). (4) **PP-INPUT-IFRAME-CLICK-NESTED**: nested `waitDocumentSeedReady` before cold `rebuildAndResync`; blueprint `input-iframe-click` PASS. | [open.md](open.md) · [extension-plane.md](extension-plane.md) · [virtual-assets.md](virtual-assets.md) · [input.md](input.md) · `virtual/bootstrap.ts` · `scripts/lab-assets-stress.js` |
| 2026-08-28 | **Launch / custom scripts LOCKED (Rodrigo)** — CDP-only in the same inject bundle as Virtual; order Virtual → customs; per-script try/catch isolation; TargetRules on every CDP target; Position removed from config/DTO/wire; remote = sidecar fetch + inline (not HTML `src`). | [browser-session.md](browser-session.md) Launch / custom scripts · `inject/buildProjectionInjectBundle.ts` · `inject/resolveLaunchScripts.ts` · `proto/browser_session.proto` |
| 2026-08-28 | **PP inject boot SEALED (Rodrigo)** — **SUPERSEDED 2026-08-29** by [runtime-redesign.md](runtime-redesign.md). Was: CDP `Page.addScriptToEvaluateOnNewDocument` + lateBoot/arm. | [browser-session.md](browser-session.md) · [runtime-redesign.md](runtime-redesign.md) |
...
| 2026-08-27 | **`sparse-cdp` input V1 SEALED (Rodrigo)** — functional cutover complete: id-addressed click + **client pointer coords** (Virtual bounds validate, CDP at pointer — not element center; fixes overlay dismiss vs dropdown mis-click on Eneba); keyboard wire `intent.key` + `preventDefault` on editables; ASCII/`Space` via `page.keyboard` (`resolveKeyboardDispatchKey` — `' '.trim()` was silently dropping Space); `historyNav` → Virtual `goBack`/`goForward` (shortcuts, `popstate` trap, touch edge-swipe); lab `resolveAndClickDomInputByNodeId` pre-resolves center when no client coords. Proof: Docker blueprints (see [input.md](input.md) bar — **10/10** as of 2026-08-28 nested iframe click close); Eneba real-site validated; units green. Deferred per §7: touch, IME, `setFiles`, hover/drag. | [input.md](input.md) · `keyboardDispatch.ts` · `bootstrap.ts` (`resolveNodeHit`) · `projectedInputCapture.ts` · `EventApplier.ts` |
| 2026-08-27 | **PP single-tab + CSP plane after locale popup (Binance-class)** — second tab forbidden per session; intercept → same-tab redirect; orphan close + adopt URL on primary. | [browser-session.md](browser-session.md) · [csp.md](csp.md) · [open.md](open.md) PP-CSP-SINGLE-TAB · `singleTab.ts` |
| 2026-08-27 | **`sparse-cdp` keyboard restored — wire `key`, editable preventDefault, full editing dispatch (Rodrigo)** — real-site symptom: text visible in Projected search but Enter did nothing on Virtual. Root cause chain: (1) `EventApplier` dispatched `intent.code` (`KeyU`) instead of `intent.key` (`u`); (2) sparse keyboard catalog rejected almost all editing keys; (3) Projected allowed browser-default mutation on editables → phantom local text. Fix: `EventApplier` uses `intent.key \|\| intent.code`; `SparseCdpKeyboardPeripheral` — single-char `insertText`, else lazy `page.keyboard.down/up`; Projected `onKey` `preventDefault` on editables. Supersedes the closed 3-key (`Enter`/`Escape`/`Tab`) sparse catalog. Lab: `input-forms-enter` blueprint. | [input.md](input.md) · `EventApplier.ts` · `sparseCdpInputAdapter.ts` · `projectedInputCapture.ts` · `input-forms-enter.json` |
| 2026-08-27 | **PP inject CDP-only cutover** — removed HTML script-tag inject (`projectionProducerDocumentMutator`, `scriptInjectMutator`, stored-script Fetch fulfill). Runtime + launch `scripts` = unified bundle via `ProjectionRuntimeInstaller` + `Page.addScriptToEvaluateOnNewDocument` per target (OOPIF via `frameCdpSession`). Sentinel scrub in prelude + `bootstrap.ts`. Closes PP-INJECT-THIRD-PARTY-MIME. | `inject/*` · [csp.md](csp.md) §10 · [open.md](open.md) |
| 2026-08-27 | **`sparse-cdp` click fail-closed: `event.target` + `idOf` only — no coord fallback (Rodrigo)** — supersedes the `nodeId == null` raw-coordinate trade-off in [input.md](input.md) §2.1a and the `elementFromPoint` + `idOfNearest` capture path from the id-addressed ruling below. Projected `runPointerEdge`: resolve `registry.idOf(event.target)` exact match; registry miss → `noteSkip('no_node')`, no enqueue. Sidecar `EventApplier` `down`/`up`: `nodeId == null` → reject `missing_node_id` / phase `validate`. Rationale: coord hit-test + nearest-ancestor walk mis-identifies targets (real-site: carousel chevron vs hamburger). Scroll/input dirty paths keep `idOfNearest` where they already use `event.target` for scrollable elements — pointer click only. | [input.md](input.md) §2.1a · `projectedInputCapture.ts` · `EventApplier.ts` · `clickDelivery.ts` |
| 2026-08-27 | **VirtualDomainBus child routing is O(1) via ChildScopeIndex — no DOM `querySelectorAll` (Rodrigo)** — CPU profile on Eneba showed ~79% self-time in `querySelectorAll` inside `forEachChildWindow`/`routeOutbound` (ad-hoc DOM scan on every bus message). Fixed to the sealed multi-doc contract: `ChildScopeIndex` keeps forward `nodeId→contextId` + reverse `contextId→nodeId` + `WeakMap` window→contextId; bootstrap wires `setChildFabric` / `setDeliverableCheck` so unicast/broadcast iterate only admitted hosts (`contentWindow` live). Hopeful broadcast on unknown dest removed — fail-closed `context_not_found`. `isDeliverableDestination` is **live index** (root `1` or `windowOf != null`), not `mintAllocator.hasMinted` (monotonic ghost). | [multi-document.md](multi-document.md) §3 · `childScopes.ts` · `virtualDomainBus.ts` · `bootstrap.ts` · `rootRuntime.ts` |
| 2026-08-27 | **PP OS input deleted from codebase — sparse-cdp is the only remaining path (Rodrigo)** ("limar COMPLETAMENTE todo o código feito pra isso"). Removed: `os-abs` adapter, `AbsOsInputStack`, ABS peripherals, S6 census stack (`ProjectedInputRuntime`, `snapshotScrollCensus`, `applyScrollCensus`, `ScrollableIndex`, `census` on `UnifiedIntent`, `census-coordinated` ClickDeliveryStrategy), OS lab scripts/suites, dual `--input-adapter` / lab UI select, `pageProjectionInputAdapterKind`, `openAbsPointer`. Remaining: sparse-cdp + live-node-resolve click + `applyScrollSet` / `resolveNodeHit`. VideoStreaming `browser/input/os/**` + `OsInputBackend` (`SPECULUM_INPUT_BACKEND`) **kept** (different product). Spec files **not** deleted — [input.md](input.md) body kept as historical record under an explicit banner; reopen OS only with redesign + new decision-log row. `PP-INPUT-VIRTUAL-MINT-GHOST` closed-by-deletion (consumer was OS census). | [input.md](input.md) · [open.md](open.md) · `sparseCdpInputAdapter.ts` · `EventApplier.ts` · `PageProjectionBrowserSession.ts` |
...
| 2026-08-27 | **Input adapter port category introduced** — "input adapter" (ports + `createInputAdapter`) is a new category, distinct from the already-purged input-v2 Mode A/B/C letters. `os-abs` (`sidecar/browser/input/adapters/osAbsInputAdapter.ts`) is the sealed v0 production adapter — 100% behaviourally identical to the pre-extraction inline `AbsOsInputStack` wiring. A second adapter is permitted only when it sits behind the same `EventApplier`, the same wire (`UnifiedIntent`), and the same S6 Phase A census law — and must never reintroduce an env-var toggle (like the purged `SPECULUM_INPUT_BACKEND`) or use CDP as the sidecar↔Virtual data-plane carrier (that stays loopback WS per E-03/E-08). | [input.md](input.md) §2.1 · `sidecar/browser/input/ports.ts` · `createInputAdapter.ts` |
...
| 2026-09-14 | **Gecko nested contextId** — mint at BC `CreateDetached`; attach this BC; `NODE_NEW` bit 7; no supervisor `ContextCreated` for iframes. Chromium `getScopeId` unchanged. | [multi-document.md](multi-document.md) §4.2 |
| 2026-09-14 | **Gecko `preTableHash` + resync** — ordinary frames stamp real pre-hash; cold attach is `resyncVirtual`; mid-session and late consumer use control `Resync` força 0 (`emitResyncFrame`). No frame buffer in the supervisor. | [frame-protocol.md](frame-protocol.md) §5.8 |
...
| 2026-08-19 | **Nested Projected resync parity** — per-context double-buffer + bounded retry ([client/nestedResyncSurface.ts](../../../sidecar/browser/mirror/projection/client/nestedResyncSurface.ts)). |
...
| Topic | Decision |
|-------|----------|
| Package | `@speculum/page-projection` at `packages/page-projection` (`private`; sidecar `file:` link; no root workspaces required) |
