...
export function RemoveInjectionPage() {
  ...
  const remove = async () => {
    ...
    try {
      ...
      navigate('/w7s/admin/scripts?tab=injections')
    } catch (err) { setError(err instanceof Error ? err.message : 'Could not apply Scripting configuration.') }
    finally { setPending(false) }
  }
  if (error && !injection) return <section className="space-y-4"><SaveFeedback mode="banner-error" message={error} /><Button onClick={() => navigate('/w7s/admin/scripts?tab=injections')}>Back to injections</Button></section>
  ...
  return <section className="mx-auto max-w-2xl space-y-6"><PageHeader title="Remove injection" description="This updates Scripting configuration. Matching pages will no longer receive this script." />
    ...
    <div className="flex justify-end gap-2"><Button variant="outline" onClick={() => navigate('/w7s/admin/scripts?tab=injections')}>Cancel</Button><Button variant="destructive" onClick={() => setOpen(true)}>Remove and apply</Button></div>
