...
import { LabConsoleMessages } from './LabConsoleMessages'
import { LabConsolePrompt } from './LabConsolePrompt'
import {
...
interface LabConsolePanelProps {
  live: boolean
  jsBridgeEnabled: boolean | null
  lines: LabConsoleLine[]
  onClear: () => void
 ...
 * One-shot Eval stays in {@link LabEvalPanel}.
 */
export function LabConsolePanel({
  live,
  jsBridgeEnabled,
  lines,
  onClear,
  onRunCommand,
}: LabConsolePanelProps) {
  ...
  return (
    <div className="flex h-full min-h-[220px] min-w-0 flex-col overflow-hidden border border-border bg-background">
      ...
      <LabConsoleMessages lines={visible} scrollerRef={scrollerRef} />
