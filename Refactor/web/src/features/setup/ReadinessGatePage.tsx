...
export function ReadinessGatePage() {
  ...
  useEffect(load, [load])
  if (config?.operational) return <Navigate to="/w7s/admin" replace />
  ...
  return <main className="mx-auto max-w-2xl space-y-6 px-6 py-12">
    ...
    {config ? <Card>
      ...
      <CardContent className="space-y-5">
        ...
        <NextBestAction title="Start guided configuration" body="Configure each missing mandatory section one step at a time." ctaLabel="Start guided configuration" href="/w7s/setup/configure" />
        {getAdminAuth() ? <Link className="text-sm underline" to="/w7s/admin">Back to Home</Link> : <p className="text-sm text-muted-foreground">You need to <Link className="underline" to="/w7s/admin/login?returnUrl=%2Fsetup%2Fconfigure">sign in to apply changes</Link> before changes can be applied.</p>}
      </CardContent>
    </Card> : null}
  </main>
}
