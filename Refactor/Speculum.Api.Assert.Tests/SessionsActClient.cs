...
/// <summary>
/// Act client for Refactor SessionHub + token-gated dev harness input/evaluate/resize.
/// </summary>
public sealed class SessionsActClient : IAsyncDisposable
{
    private readonly SessionsAssertHost _host;
    ...
    public SessionsActClient(SessionsAssertHost host) => _host = host;
    ...
    public async Task SendInputAsync(string type, string payload, CancellationToken ct = default)
    {
        ...
        using var response = await _http.PostAsJsonAsync(
            $"{_host.ApiBase}/api/dev/sessions/{_sessionId}/input",
    ...
    public async Task<string> EvaluateAsync(string expression, CancellationToken ct = default)
    {
        EnsureSession();
        using var response = await _http.PostAsJsonAsync(
            $"{_host.ApiBase}/api/dev/sessions/{_sessionId}/evaluate",
