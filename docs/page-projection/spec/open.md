...
| **GECKO-ASSET-SVG-MIME** | Projected logo/SVG `complete && naturalWidth=0` → alt text blows header (~344px), overlaps. Cause: tee served incomplete body and/or empty Content-Type; Chromium refuses SVG without MIME. | **Fixed 2026-09-16.** Asset registry: wait for tee complete (no early emit); NoteMime on stop; sniff SVG/JPEG/PNG/WebP; lab client MIME sniff defense. **Prova same-S:** `brokenImgs=0`, header **60px** (was 344). Residual: some imgs still `complete:false` at 35–40s (in-flight); overlapPairs metric still high — not accept. |
