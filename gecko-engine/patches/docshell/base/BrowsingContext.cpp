...
already_AddRefed<BrowsingContext> BrowsingContext::CreateDetached(
  ...
  // Speculum: C no snapshot de nascimento. Sem pai / chrome / print / Top
  // sem projeção → 0. CreateFromIPC só herda. Não Set depois.
  fields.Get<IDX_SpeculumContextId>() = SpeculumMintNestedIfProjected(
      parentBC.get(), aType == Type::Content, aOptions.isForPrinting);
