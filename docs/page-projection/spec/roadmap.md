...
| V4 algorithm `Refactor/sidecar/browser/mirror/projection/` (not `lab/`) | **DONE** for **DOM table**, open named shadow, CSSOM for constructed `adoptedStyleSheets` + `CSSStyleRule` (including admitted roots), form `PROP_SET`, **lab same-origin nested iframe**. Lab is a **caller**. CLI `--iso`: O2 local + Node table×table + CSSOM O2. Tree×tree = lab UI with a DOM client. Nested tree iso = UI `iframe-open` (CLI without DOM client fails `iso.nested`). Off-tree kinds **LOCKED** ([subtrees.md](subtrees.md)). Shadow **shipped** ([shadow.md](shadow.md)). Nested SO **lab shipped** ([multi-document.md](multi-document.md)). **Not** input. **Not** canvas. |
...
| 5 | **OPEN-6 nested browsing contexts** | Product | **YES** (production + XO/NIT) | Feature 2 of [subtrees.md](subtrees.md). **Lab same-origin iframe shipped** (`iframe-open`). XO / srcdoc / sandbox / fenced NIT. Design: [multi-document.md](multi-document.md). |
