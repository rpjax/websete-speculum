...
export type DocumentReadyPage = {
  url(): string;
  evaluate(pageFunction: string): Promise<unknown>;
  on(event: 'framenavigated', handler: (frame: { parentFrame: () => unknown }) => void): void;
  off(event: 'framenavigated', handler: (frame: { parentFrame: () => unknown }) => void): void;
};

export type DocumentReadyOptions = {
  page: DocumentReadyPage;
  isStopped: () => boolean;
  /** Current document epoch token (may change while waiting). */
  getDocumentEpoch: () => string | null;
  timeoutMs?: number;
  growthQuietMs?: number;
  fingerprintQuietMs?: number;
  watchAfterQuietMs?: number;
};
...
export async function waitVirtualDocumentReady(opts: DocumentReadyOptions): Promise<void> {
  ...
  let navBump = 0;
  const onFrameNavigated = (frame: { parentFrame: () => unknown }) => {
    if (frame.parentFrame() == null) navBump += 1;
  };
  ...
  const navigationChanged = (): boolean => {
    ...
    if (!bumped && url === seenUrl && epoch === seenEpoch) return false;
    lastNavBump = navBump;
    seenUrl = url;
    seenEpoch = epoch;
    resetClocks();
  ...
  try {
    while (!opts.isStopped() && Date.now() - t0 < timeoutMs) {
      ...
      if (maxLen > 0 && Date.now() - lastGrowthAt >= growthQuietMs) {
        ...
        const botWall =
          maxLen < 64 * 1024
          && /access denied|attention required|just a moment|verify you are|um momento|acesso negado/i.test(
            title,
          );
        if (!botWall || Date.now() - t0 >= timeoutMs - 3_000) break;
        lastGrowthAt = Date.now();
      }
      await new Promise((r) => setTimeout(r, 200));
    }
