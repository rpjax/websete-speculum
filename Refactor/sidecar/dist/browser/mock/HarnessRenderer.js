...
exports.HarnessRenderer = void 0;
...
exports.HarnessRenderer = HarnessRenderer;
function paint(ctx, width, height, snap) {
    const toolbarH = 44;
    const pad = 12;
    // Pulsing background
    ...
    const lag = snap.lastInputAt > 0 ? `${Math.max(0, snap.nowMs - snap.lastInputAt)}ms since input` : 'no input yet';
    ctx.fillText(`MOCK HARNESS  fps=${snap.emitFps.toFixed(0)}  encode=${snap.encodeMs}ms  q=${snap.jpegQuality}`, pad + 10, hudY + 10);
    ctx.fillText(`inputs=${snap.inputCount}  last=${snap.lastInputType || '—'}  ${lag}`, pad + 10, hudY + 28);
    // Held keys
...
function drawButton(ctx, hit, label, enabled) {
...
//# sourceMappingURL=HarnessRenderer.js.map