...
class Screencast {
    ...
    _frameId = 0;
    constructor() { }
    static async start(page, width, height, onFrame) {
        const sc = new Screencast();
        // Fire the capture loop in the background — do not await.
        sc._run(page, onFrame).catch(err => {
            if (!sc._stopped) {
                console.error('[Screencast] fatal loop error:', err.message);
            }
        });
        return sc;
    }
    async _run(page, onFrame) {
        let prevHash = '';
        ...
        while (!this._stopped) {
            ...
            try {
                ...
                if (this._stopped)
                    break;
                console.error('[Screencast] screenshot error:', err.message);
                await sleep(200);
                continue;
            }
            if (this._stopped)
...
function targetFrameMs(skips) {
    if (skips >= THROTTLE_IDLE)
        return 1000 / FPS_IDLE;
    if (skips >= THROTTLE_MODERATE)
        return 1000 / FPS_MODERATE;
    return TARGET_MS;
}
function sleep(ms) {
