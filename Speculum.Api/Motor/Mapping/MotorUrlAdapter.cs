...
public sealed class MotorUrlAdapter
{
        ...
        string motorRequestHost)
    {
        ...
        var clientQuery = string.IsNullOrEmpty(siteQuery)
            ? $"?{W7sNavigationQueryParam.Name}={nso}"
            : $"{siteQuery}&{W7sNavigationQueryParam.Name}={nso}";
    ...
    private static (string Path, string Query) StripNsoFromClientQuery(Uri uri)
    {
        ...
        var kept = parts.Where(p => !p.StartsWith(W7sNavigationQueryParam.Name + "=", StringComparison.Ordinal))
                        .ToArray();
    ...
    private NavigationStateV1? ExtractNso(Uri uri)
    {
        ...
        {
            if (part.StartsWith(W7sNavigationQueryParam.Name + "=", StringComparison.Ordinal))
            {
                var value = part[(W7sNavigationQueryParam.Name.Length + 1)..];
