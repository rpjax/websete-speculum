...
function validateBlueprint(bp) {
    ...
    for (const q of bp.queues) {
        for (const a of q.actions) {
            if (actions.has(a.id))
                return { ok: false, reason: `duplicate action id ${a.id}` };
            actions.set(a.id, { ...a, queue: a.queue ?? q.name });
        }
    }
    if (actions.size === 0)
        return { ok: false, reason: 'blueprint has no actions' };
    for (const a of actions.values()) {
        for (const dep of a.dependsOn ?? []) {
            if (!actions.has(dep))
                return { ok: false, reason: `dependsOn unknown id ${dep} from ${a.id}` };
        }
        for (const dep of a.awaits ?? []) {
            if (!actions.has(dep))
                return { ok: false, reason: `awaits unknown id ${dep} from ${a.id}` };
        }
    }
    ...
    for (const id of actions.keys()) {
        const c = dfs(id);
        if (c)
            return { ok: false, reason: `cycle involving ${c}` };
    }
    ...
    const orderedBefore = (a, b) => {
        if (reaches(b, a))
            return true; // b depends on a ⇒ a before b
