...
import type { LabLogEntry, LabLogLevel } from './labLog'

const LEVEL_VARIANT: Record<LabLogLevel, 'default' | 'success' | 'warning' | 'destructive' | 'muted'> = {
  info: 'default',
  wire: 'muted',
  warn: 'warning',
  error: 'destructive',
...
export function LabEventFeed({ entries }: { entries: LabLogEntry[] }) {
  if (entries.length === 0) {
    return (
